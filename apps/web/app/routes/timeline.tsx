...
export default function Timeline() {
  ...
  const CoverMosaic = ({ covers }: { covers: Bucket["coverAssets"] }) => (
    <div className={`grid ${covers.length >= 4 ? "grid-cols-2" : "grid-cols-1"} gap-0.5 w-full aspect-square overflow-hidden rounded-xl bg-muted/40`}>
      {(covers.length >= 4 ? covers.slice(0, 4) : covers.slice(0, 1)).map((c) => (
          ...
          <div className="max-w-5xl mx-auto grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
            {(yearBuckets ?? years.map(([year, { count }]) => ({ period: `${year}-01-01T00:00:00.000Z`, count, coverAssets: [] }))).map((bucket) => {
              ...
              return (
                ...
                >
                  <CoverMosaic covers={bucket.coverAssets} />
