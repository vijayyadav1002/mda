...
export async function getCacheStats(): Promise<CacheStats> {
  ...
  const [thumbnails, previews, hls, transcoded] = await Promise.all([
    measure(path.resolve(config.thumbnailCachePath), false, config.thumbnailCacheMaxBytes, 'Thumbnails'),
    measure(path.join(cacheBasePath, 'previews'), false, config.previewCacheMaxBytes, 'Previews'),
    measure(path.join(cacheBasePath, 'hls'), true, config.hlsCacheMaxBytes, 'HLS'),
    measure(path.join(cacheBasePath, 'transcoded'), false, config.transcodedCacheMaxBytes, 'Transcoded'),
  ]);
